...
## Created by: NAME
## Prep coefficient draws for oaxaca-blinder decomp for ASHER: by country using pre partum mod contra
## set-up --------
...
create_draws <- function(cur_country){
  ...
  coef_dt <- fread(file.path(in.dir, paste0('alt_', mode_use, '_oaxaca_15_19_ci_detail_endline_marriage_preg_drop_outcome_c_', cur_country, '.csv')), fill = T)
  ...
  write.csv(coef_draws_dt, file.path(out.dir, paste0(cur_country, '_coef_draws_marriage_preg_drop_outcome_c.csv')), row.names=F)
}
create_draws('gh')
create_draws('mw')
create_draws('np')
