...
run_decomp <- function(cur_country){
  ...
  if (cur_country == 'mw'){
    endline <- 'mics'
    # remove outcome
    draws_dt <- draws_dt[, -c("outcome_c_mics")]
  } else {
    endline <- 'dhs'
    draws_dt <- draws_dt[, -c("outcome_c_dhs")]
  }

   # read in coefficients from OB run: use outcome c 
  coef_dt <- fread(file.path(in.dir.ob, paste0("alt_", endline, "_oaxaca_15_19_ci_detail_endline_outcome_c_", cur_country, ".csv")), fill = T)
  ...
  for (region in unique(draws_dt$admin_1_region)) {
    ...
    data_draws_dt_tmp <- data_draws_dt_tmp[, -c("admin_1_region", "country", "wealth_dummies1")]
