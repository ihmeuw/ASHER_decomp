...
## create diagnostic plots for religion in ASHER decomp: using mics as endline 
rm(list=ls())
...
hh_dt <- fread(file.path(in.dir, 'hh_table_mics.csv'))
...
for (cur_country in c("cm", "mw", "np", "rw",'gh')){
  ...
  title <- paste0('religious_breakdown_', cur_country, '.png')
