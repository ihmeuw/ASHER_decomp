...
## create diagnostic plots for method mix in ASHER decomp: using DHS as malawi endline 
rm(list=ls())
...
## 15-19 ---------
all_ind_means_15_19 <- data.table()
...
for (cur_country in countries) {
  ...
  dhs_svy <- svydesign(id= ~psu, strata = ~strata, weights = ~pweight, data = ind_dt[country ==cur_country & age %in% c(seq(15,19)) ], nest = TRUE)
  means_p1 <- svyby(~current_method, ~year, subset(dhs_svy, year == first_year) , svymean, na.rm =T) 
  means_p2 <- svyby(~current_method, ~year, subset(dhs_svy, year == last_year) , svymean,na.rm =T)
  ...
  mean_se_p2 <- organize_mean_se_multiple(means_p2_long)
  
  mean_se_all <- rbind(mean_se_p1, mean_se_p2)
  mean_se_all[, country := cur_country]
  all_ind_means_15_19 <- rbind(all_ind_means_15_19, mean_se_all)
}

## rename current method none to reflect unmet need instead
all_ind_means_15_19[, variable := str_replace(variable, 'current_method', '')]
all_ind_means_15_19[, variable := ifelse(variable %in% c("none", "blank"), 'unmet_need', variable)]

## combine calendar methods into rhythm 
all_ind_means_15_19[, variable := ifelse(variable == "calendar_methods", 'rhythm', variable)]
...
all_ind_means_15_19[, country_lab := unlist(country_labs[country])]

# method labels
all_ind_means_15_19[, method_lab := unlist(method_labs[variable])]
all_ind_means_15_19[, method_lab := factor(method_lab, levels = unlist(unname(method_labs)))]

all_ind_means_15_19[, baseline := min(year), by = "country"]
all_ind_means_15_19[, latest := max(year), by = "country"]
all_ind_means_15_19[, year_type := ifelse(year == baseline, "baseline", "latest")]
...
ggsave(file.path(out.dir, 'method_mix_all_15_19_dhs.png'), width = 10, height =10)
...
title <- paste0('method_mix_15_19_', cur_country, '_dhs.png')
ggsave(file.path(out.dir, title), width = 10, height =10)
}

## 15-24 ---------
all_ind_means_15_24 <- data.table()
countries <- c("cm", "mw", "np", "rw",'gh')
for (cur_country in countries) {
  options(survey.adjust.domain.lonely=TRUE)
  options(survey.lonely.psu="adjust")
  ...
  first_year <- min(current_data$year)
  last_year <- max(current_data$year)
  dhs_svy <- svydesign(id= ~psu, strata = ~strata, weights = ~pweight, data = ind_dt[country ==cur_country & age %in% c(seq(15,24)) ], nest = TRUE)
  means_p1 <- svyby(~current_method, ~year, subset(dhs_svy, year == first_year) , svymean, na.rm =T) 
  means_p2 <- svyby(~current_method, ~year, subset(dhs_svy, year == last_year) , svymean,na.rm =T)
  ...
  mean_se_all <- rbind(mean_se_p1, mean_se_p2)
  mean_se_all[, country := cur_country]
  all_ind_means_15_24 <- rbind(all_ind_means_15_24, mean_se_all)
}

## rename current method none to reflect unmet need instead
all_ind_means_15_24[, variable := str_replace(variable, 'current_method', '')]
all_ind_means_15_24[, variable := ifelse(variable %in% c("none", "blank"), 'unmet_need', variable)]

## combine calendar methods into rhythm 
all_ind_means_15_24[, variable := ifelse(variable == "calendar_methods", 'rhythm', variable)]
all_ind_means_15_24[, country_lab := unlist(country_labs[country])]
## combine foam jelly sponge into other modern 
all_ind_means_15_24[, variable := ifelse(variable == "foam_jelly_sponge", 'other_modern_method', variable)]

all_ind_means_15_24[, method_lab := unlist(method_labs[variable])]
all_ind_means_15_24[, method_lab := factor(method_lab, levels =unique(unlist(unname(method_labs))))]

all_ind_means_15_24[, baseline := min(year), by = "country"]
all_ind_means_15_24[, latest := max(year), by = "country"]
all_ind_means_15_24[, year_type := ifelse(year == baseline, "baseline", "latest")]
...
ggsave(file.path(out.dir, 'method_mix_all_15_24_dhs.png'), width = 10, height =10)
...
for (cur_country in c("cm", "mw", "np", "rw",'gh')){
  ...
  title <- paste0('method_mix_15_24_', cur_country, '_dhs.png')
  ggsave(file.path(out.dir, title), width = 10, height =10)
}
## 15-49 ---------
all_ind_means_15_49 <- data.table()
countries <- c("cm", "mw", "np", "rw",'gh')
for (cur_country in countries) {
  options(survey.adjust.domain.lonely=TRUE)
  options(survey.lonely.psu="adjust")
  ...
  first_year <- min(current_data$year)
  last_year <- max(current_data$year)
  dhs_svy <- svydesign(id= ~psu, strata = ~strata, weights = ~pweight, data = ind_dt[country ==cur_country & age %in% c(seq(15,49)) ], nest = TRUE)
  means_p1 <- svyby(~current_method, ~year, subset(dhs_svy, year == first_year) , svymean, na.rm =T) 
  means_p2 <- svyby(~current_method, ~year, subset(dhs_svy, year == last_year) , svymean,na.rm =T)
  ...
  mean_se_all <- rbind(mean_se_p1, mean_se_p2)
  mean_se_all[, country := cur_country]
  all_ind_means_15_49 <- rbind(all_ind_means_15_49, mean_se_all)
}


## rename current method none to reflect unmet need instead
all_ind_means_15_49[, variable := str_replace(variable, 'current_method', '')]
all_ind_means_15_49[, variable := ifelse(variable%in% c("none","blank"), 'unmet_need', variable)]

## combine calendar methods into rhythm 
all_ind_means_15_49[, variable := ifelse(variable == "calendar_methods", 'rhythm', variable)]
## combine foam jelly sponge into other modern 
all_ind_means_15_49[, variable := ifelse(variable == "foam_jelly_sponge", 'other_modern_method', variable)]


all_ind_means_15_49[, country_lab := unlist(country_labs[country])]


# method labels
all_ind_means_15_49[, method_lab := unlist(method_labs[variable])]
all_ind_means_15_49[, method_lab := factor(method_lab, levels =unique(unlist(unname(method_labs))))]

all_ind_means_15_49[, baseline := min(year), by = "country"]
all_ind_means_15_49[, latest := max(year), by = "country"]
all_ind_means_15_49[, year_type := ifelse(year == baseline, "baseline", "latest")]
...
ggsave(file.path(out.dir, 'method_mix_all_15_49_dhs.png'), width = 10, height =10)
...
for (cur_country in c("cm", "mw", "np", "rw",'gh')){
  ...
  title <- paste0('method_mix_15_49_', cur_country, '_dhs.png')
  ggsave(file.path(out.dir, title), width = 10, height =10)
}
