...
## create diagnostic plots for age at first birth in ASHER decomp: MICS as endline MWI
rm(list=ls())
...
## read in ind summary data
ind_dt <- fread(file.path(in.dir, 'ind_table_mics.csv'))

## filter to just age_1st_birth variables
ind_dt <- ind_dt[merge_var == 'age_1st_birth']

## rename age_1st_birth variable
ind_dt[, variable := str_replace(variable, 'age_1st_birth', '')]

## reshape wide to long
setnames(ind_dt, c("mean_baseline", "mean_latest", "variable"), c("baseline", "endline", "variable_name"))
ind_dt_long <- melt(ind_dt, id.vars = c("country", "variable_name"),
             measure.vars = c("baseline", "endline"))
setnames(ind_dt_long, "variable", "year_type")
...
ind_dt_long[, country_lab := unlist(country_labs[country])]

## plot each country separately
for (cur_country in c("cm", "mw", "np", "rw",'gh')){
  print(cur_country)
  colourCount <- length(unique(ind_dt_long[country == cur_country]$variable_name))
  getPalette <- colorRampPalette(brewer.pal(9, "Set3"))
  
  ggplot(data= ind_dt_long[country == cur_country], aes(x = year_type, y = value, fill = variable_name))+
    scale_y_continuous("Percent of population", label = percent, expand = c(0,0)) +
    geom_bar(stat = 'identity')+
    scale_fill_manual(values = getPalette(colourCount))+
    labs(fill = "Age at first birth") +
    ...
    ggtitle(paste("Age at first birth,", unique(ind_dt_long[country == cur_country]$country_lab)))
  
  title <- paste0('age_1st_birth_', cur_country, '.png')
