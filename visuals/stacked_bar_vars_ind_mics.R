...
## create diagnostic plots a series of variables in the ASHER decomp for baseline and endline: individual level data: with mics as endline
rm(list=ls())
...
## read in data
merged_dt <- fread(file.path(in.dir, 'merged_baseline_endline_mics.csv'))
...
continuous_vars <- c("age", "age_1st_sex_imp","gap_sex_mar", "births_3_yr", "births_5_yr")

prop_vars <- c("curr_cohabit", "desire_child_teen", "fp_exp_media", 
               "mcpr", "never_had_intercourse", "attend_school")
...
create_data <- function(var, age_start, age_end){
  ...
  variable_labs <- list("age" = "Age",
                        ...
                        "attend_school" = "Percent attending school in the current year",
                        "births_3_yr" = "Number of births in the past 3 years",
                        "births_5_yr" = "Number of births in the past 5 years")
...
## here open pdf
pdf(file = file.path(out.dir, 'diagnostic_vars_asher_mics.pdf'))
...
plot_continuous_diff <- function(data, age_start, age_end){
  ...
  gg <- ggplot(data= data_wide, aes(x = country_lab, y = diff))+
    scale_y_continuous(paste0("Difference in mean value, ", age_start, "-", age_end), expand = c(0,0), limits = c(min_val, max_val+1)) +
    geom_bar(stat = 'identity', aes(fill = country_lab))+
    theme_bw()   + theme(panel.spacing.x = unit(4, "mm"),
                         legend.position = "none", strip.text = element_text(size = 12),
                         plot.title = element_text(size = 16), axis.title = element_text(size = 12),
...
plot_prop_diff <- function(data, age_start, age_end){
  ...
  max_val <- max(data_wide$diff) 
  max_val <- ifelse(max_val <0, 0, max_val)
