...
create_draws <- function(cur_country){
  ...
  coef_dt <- fread(file.path(in.dir, paste0('outcome_c_dhs_oaxaca_15_19_ci_detail_endline_contra_', cur_country, '.csv')), fill = T)
  ...
  write.csv(coef_draws_dt, file.path(out.dir, paste0(cur_country, '_coef_draws_w_contra.csv')), row.names=F)
}
create_draws('gh')
create_draws('mw')
create_draws('np')
