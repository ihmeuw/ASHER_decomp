...
# Prep data draws from baseline and endline for oaxaca-blinder decomp for ASHER to use in analysis using pooled data for regression (sensitivity analysis): among sexually active adolescents only 
# clear memory
rm(list=ls())
...
# filter to those who have had intercourse
ob_dhs_dt <- ob_dhs_dt[had_intercourse==1]
ob_mics_dt <- ob_mics_dt[had_intercourse==1]
...
create_draws <- function(cur_country){
  ...
  for (var in variables){
    ...
    write.csv(draws_diff, file.path(out.dir, paste0(cur_country, '_baseline_endline_draws_part_2.csv')), row.names=F)
}
