...
# load packages
pacman::p_load(data.table,magrittr,tidyverse,parallel,plyr,dplyr,haven,survey,tools,devtools,scales)
...
for (cur_country in countries) {
  ...
  if (baseline_endline == T) {
    ...
    wq_files <- list.files("FILEPATH", full.names = T)
  ...
  gg <- ggplot(data = plot_dt, aes(x = variable, y = survey, fill = value)) +
    ...
    scale_fill_distiller(palette = "RdYlGn", labels = scales::percent, direction = 1) +
