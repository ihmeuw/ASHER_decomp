...
shp2 <- st_read(file.path("FILEPATHlbd_standard_admin_2.shp"))
...
get_admin_1_means <- function(dt_file, cur_country){
  ...
  variables <- c(variable_list$Variable, "country", 'year','any_birth_preg_2_yr_dhs', 'any_birth_preg_2_yr_mics', 'cluster', 
                 'pweight', 'survey', 'mean_yrs_schooling_head', 'mcpr', 'wealth_dummies1', 'wealth_dummies2', 'wealth_dummies3',
                 'wealth_dummies4', 'wealth_dummies5')
  
  dt <- dt[,..variables]
  dt <- dt[age %in% c(seq(15,19))]
  # list of variables to take the mean of 
  variables_means <- c(variable_list$Variable,'any_birth_preg_2_yr_dhs', 'any_birth_preg_2_yr_mics', 'wealth_dummies1', 'wealth_dummies2', 'wealth_dummies3',
                       'wealth_dummies4', 'wealth_dummies5')
